...
function [PI_i, PI_p, T_i, T_p, err] = SolveInDirectProb(iter_max, ParaInPlane, ParaParking)
    %.. Assume Perfect Availability for initialization
    nx_i = ParaInPlane.Q + ParaInPlane.R + 1;
    Pdav = 0.9*ones(1,nx_i);
    [PI_i] = SolveInDirectPlane(Pdav, ParaInPlane);
    ...
    for iter = 1:iter_max
        %.. Update the solution
        [PI_i, T_i] = SolveInDirectPlane(PI_p.Pdav, ParaInPlane);
